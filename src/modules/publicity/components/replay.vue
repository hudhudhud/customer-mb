        ...
        <div  class="oth-list-item">
            <div class="left">
                <div class="info">
                    ...
                    <!--作者或评论者自己才可以回复 -->
                    <template v-if="userId&&canReplay">
                      <div @click="replay(item)" class="replay-btn" v-if="(isAuthor||(mainCommentUserId&&mainCommentUserId.toLocaleLowerCase()==userId.toLocaleLowerCase()))">回复</div>
                    </template>
                </div>
            </div>
            <div class="right">
...
export default {
	  name:"replay",
    props:["item","mainCommentUserId","authorId","isAuthor","userId","canReplay","parentItemList"],
    methods:{
        //点赞
        praise(it){
         this.$emit('praise',it)
